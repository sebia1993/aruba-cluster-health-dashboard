...
- 연결 관련 설정을 저장하면 MM과 모든 응답 가능한 Controller의 새 지문을 한
  화면에 표시합니다. 사용자가 네트워크 담당자가 보유한 값과 모두 비교해
  승인한 키만 앱 전용 `%LOCALAPPDATA%\ArubaMiniDashboard\known_hosts`에 하나의
  원자적 갱신으로 기록합니다.
- 저장된 키와 다른 키는 자동 승인·교체하거나 Fallback으로 우회하지 않습니다.
- 승인 후에는 strict SSH 검증을 다시 적용해 MM 로그인과 최소 1대의 Cluster
  Controller 로그인이 성공해야 설정을 확정합니다. 다른 Controller가 응답하지
  않거나 인증에 실패하면 자격 증명을 전송할 준비가 안 된 장비로 구분해 경고합니다.
- 연결과 무관한 운영·알림·화면 설정만 바뀌면 장비 접속 없이 저장합니다. 전체
  연결 상태를 다시 확인하는 작업은 `고급 진단 > 연결 다시 확인`에서 실행합니다.

호스트 키 변경은 장비 교체·재설치일 수도 있고 중간자 공격 징후일 수도
있습니다. 원인을 확인하지 않은 채 `known_hosts`를 삭제하거나 교체하지
마십시오.

Paramiko 5.0.0이 구형 장비와 안전한 KEX·호스트 키·서명 알고리즘을 협상하지
못하면 비식별 `SSH_ALGORITHM_INCOMPATIBLE` 오류로 처리합니다. 호환성을 위해
제거된 약한 legacy 알고리즘을 자동으로 다시 활성화하지 않습니다. 실제 ArubaOS
버전별 호환성은 승인된 사내 환경에서 별도로 확인해야 합니다.
...
기존 SQLite의 기준값·탐지 상태·확인 대기 변화·활성 장애를 시작 시 완전하게
읽지 못하면 빈 상태로 간주하지 않습니다. 파일 저장소를 닫고 자동 점검이 꺼진
격리 메모리 저장소로 전환해 원본 DB에 후속 쓰기가 섞이지 않도록 합니다.
미등록 inventory 정리와 관련 JSON·정수 검증은 같은 transaction에서 수행하며,
검증 실패 시 삭제와 유지보수 marker가 함께 rollback됩니다. 활성 장애는
10,000건을 초과하거나 동일 논리 장애의 활성 행이 중복될 경우 부분 복원·임의
선택 대신 실패 안전하게 거부합니다.

SQLite 시작 검증은 열 이름뿐 아니라 기본 키, `NOT NULL`, `CHECK`, 필수 인덱스의
열·정렬 순서와 partial 조건을 확인합니다. 저장 JSON은 256KiB, 깊이 32,
노드 20,000개 한도를 두고 중복 키, 비유한 수, 잘못된 시간·자료형을 거부합니다.
비밀 필드 이름은 snake_case·kebab-case·camelCase를 토큰화해 어느 위치에
있든 검사하며, 상태 식별자인 `event_token`과 `durable_event_token`만 명시적으로
예외 처리합니다.
...
- 등록된 사용자명·비밀번호·Enable Secret과 흔한 key/value 비밀 형식은 공통
  마스커를 통과합니다.
- Authorization Bearer/Basic은 scheme 뒤 credential 전체를 마스킹합니다.
- 로그 파일 잠금이나 쓰기 실패 시 logging의 기본 오류 처리기가 마스킹 전
  record 본문을 stderr에 다시 출력하지 못하도록 일반 오류만 남깁니다.
...
종료 요청은 새 수동·자동 점검과 연결 테스트를 차단하고 재시도 대기를 깨운
뒤 앱이 소유한 연결 중인 IPv4 TCP socket과 활성 SSH 전송을 닫습니다. 이 정리는
GUI 밖에서 실행하며 worker thread나 프로세스를 강제로 종료하지 않습니다.
SSH 호스트 키 승인 화면이 보관하는 일회성 연결 설정은 승인 거절, 저장 실패,
재시도 실패 또는 프로그램 종료 시 즉시 폐기합니다. 새 자격 증명은 지문 승인과
필수 로그인 확인이 끝나기 전에 Credential Manager나 세션 저장소에 기록하지 않아
입력값이 불필요하게 남지 않도록 합니다.
...
2026-08-13 기준 `requirements-lock.txt`의 런타임·빌드 의존성 33개 정확 버전을
GitHub Advisory Database의 공개 공지와 대조했으며 일치하는 활성 공지는 0건이었습니다.
별도의 소스 빌드 백엔드는 확인 중 발견한 공지
`GHSA-5rjg-fvgr-3xxf`, `GHSA-h35f-9h28-mq5c`, `GHSA-8rrh-rw8j-w5fx`의
영향을 받지 않도록 `setuptools==84.0.0`, `wheel==0.46.2`로 고정합니다.

이 결과는 해당 날짜의 공개 데이터베이스 스냅샷이며 이후 새 공지나 영향 범위
변경 가능성을 배제하지 않습니다. 실제 Aruba 장비·사내망 호환성이나 Windows
실행 환경 검증을 의미하지 않으므로 릴리스마다 다시 확인해야 합니다.


## v0.6.0 장애조치 전송·저장 경계

변경 명령은 SQLite 예약이 성공한 뒤에만 쓰기를 시도합니다. 예약, 쓰기 시도,
쓰기 반환과 응답 관측을 구분하고 결과가 불명확하면 자동 재전송하지 않습니다.
재분배 직전 동일 SSH 세션에서 Leader Membership과 MM 전체 Up을 검증합니다.
모든 장애조치 SSH 연결은 하나의 Operation Registry에서 취소되며 보고서에는
비밀번호, Enable Secret, Credential ID 또는 전체 SSH 원문을 저장하지 않습니다.
