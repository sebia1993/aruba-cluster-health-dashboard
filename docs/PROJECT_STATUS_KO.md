...
아래 항목은 구현 미완료를 의미하지 않습니다. 자동화 테스트만으로 증명하지 않는
외부 환경 의존 항목입니다.

- [ ] 실제 Aruba MM의 비식별 `show switches` 출력 대조
...
완료 여부를 갱신할 때 실제 수행한 근거가 있는 항목만 체크합니다. 실제 주소,
장비명, 계정, SSH 지문과 CLI 원문은 현장 검증 근거로 사용하더라도 공개 저장소에
기록하지 않습니다. 특정 ArubaOS의 약한 legacy 알고리즘은 자동 활성화하지 않으며,
안전한 SSH 협상 실패는 명시적 호환성 오류로 처리합니다.
