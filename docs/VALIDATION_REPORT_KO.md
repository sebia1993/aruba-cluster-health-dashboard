...
| 실제 Aruba MM / 7240XM 읽기 전용 동작 | ⚠️ 별도 현장 증거 | 자동 fixture로 대체하지 않음 |
| Python 미설치 Windows 11 운용 | ⚠️ 별도 현장 증거 | 실제 PC/VM 검증 필요 |

> 자동 테스트가 실제 Aruba 장비 호환성을 증명한다고 표현하지 않습니다. 자동 검증은
> Parser·상태 전이·저장·UI·패키지 계약과 반복 장애 뒤의 프로그램 상태 복구를
> 재현 가능하게 검증합니다.
...
실제 MM/7240XM의 읽기 전용 연결·수집과 버전별 legacy 알고리즘 호환성은 별도
현장 증거 대상으로 남깁니다. 알고리즘을 추측해 자동 활성화하지 않으며, 안전한
협상이 불가능하면 `SSH_ALGORITHM_INCOMPATIBLE`로 종료하는 지원 경계를 유지합니다.
...
## 10. 별도 현장 증거와 외부 환경 경계

실제 Aruba MM/7240XM의 접속, 읽기 전용 세 명령 수집과 화면 동작을 확인했다는
공개 근거는 현재 없습니다. 이번 patch의 자동 검증은 프로그램 내부의 반복 실패
복구를 강화한 근거이며 실제 장비 호환성을 대체하지 않습니다.

| 항목 | 상태 |
|---|---|
| MM / 7240XM 연결과 읽기 전용 수집 | ⚠️ 별도 현장 증거 필요 |
| 실제 주소·장비명·SSH 지문·CLI 원문 | 공개 저장소에 기록 금지 |
...
아래 형식은 실제 검증을 완료하고 근거를 보유한 뒤에만 사용합니다. 현재 완료
상태를 뜻하지 않습니다.
