...
- `v0.4.2`: 7240XM Membership 표의 `Connection-Type`과 동적 `STATUS` 열을
...
```text
ArubaMiniDashboard-v0.4.2-windows-x64.zip
ArubaMiniDashboard-v0.4.2-windows-x64.zip.sha256
```
...
git switch main
git pull --ff-only
git tag -a v0.4.2 -m "Aruba Mini Dashboard v0.4.2"
...
- `tag`: 이미 origin에 존재하는 annotated tag. 예: `v0.4.2`
- `release_mode`:
